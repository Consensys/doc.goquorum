...
__To support private transactions in GoQuorum, the `web3.eth.sendTransaction(object)` API method has been modified.__
...
__To support sending raw transactions in GoQuorum, the `web3.eth.sendRawPrivateTransaction(string, object)` API method has been created.__

```js
web3.eth.sendRawPrivateTransaction(signedTransactionData [, privateData] [, callback])
```

Sends a pre-signed transaction, for example using [`SilentCicero/ethereumjs-accounts`](https://github.com/SilentCicero/ethereumjs-accounts).
...
!!! example
    ...
    **If the callback URL is provided**

    the following response will be received after
