...
If the peer count is 0, check that [`static-nodes.json` was updated with the correct port numbers](#3-update-ip-and-port-numbers) and [copied to the `data` directory for each node](#4-copy-static-nodes-file-and-node-keys-to-each-node).
