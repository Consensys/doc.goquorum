...
All participants listed in `privateFor` must have their [private transaction managers](../concepts/privacy-index.md#private-transaction-manager) running when the private transaction is sent. Otherwise, the transaction doesn't propagate and an error is returned.
